<?
	...
	function picture_color_allocate($image, $hexcolor){
		...
		$color = ImageColorAllocate($image, hexdec($first), hexdec($second), hexdec($third));
	...
	function ImageSmartSize($image, $size){
		...
		ImageCopyResampled($thumb, $image, 0, 0, 0, 0, $width, $height, $img_width, $img_height);
	...
	function ImagePadding($paras){
		...
		if ($paras["keep_size"]){
			...
			ImageCopyResampled($newimage, $image, $padding_x, $padding_y, 0, 0, $copyx, $copyy, $xcount, $ycount);
		}else{
			ImageCopyResampled($newimage, $image, $padding_x, $padding_y, 0, 0, $xcount, $ycount, $xcount, $ycount);
		}
	...
	function ImageEqualSizes($paras){
		...
		ImageFill($newimage, 0, 0, $color);
		ImageCopyResampled($newimage, $image, $start_x, $start_y, 0, 0, $xcount, $ycount, $xcount, $ycount);
