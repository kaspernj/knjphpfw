...
class GtkSettingsWindow
{
    ...
    function __construct(GtkWindow $win, $name)
    {
        ...
        if ($this->winsettings) {
            if (is_numeric($this->winsettings->get("size_width"))
                && is_numeric($this->winsettings->get("size_height"))
            ) {
                $this->win->resize(
                    $this->winsettings->get("size_width"),
                    $this->winsettings->get("size_height")
                );
            }

            if (is_numeric($this->winsettings->get("pos_left"))
                && is_numeric($this->winsettings->get("pos_top"))
            ) {
                $this->win->move(
                    $this->winsettings->get("pos_left"),
                    $this->winsettings->get("pos_top")
                );
            }
